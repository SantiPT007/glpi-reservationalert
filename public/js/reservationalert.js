...
<svg width="20" height="20" viewBox="0 0 24 24" fill="none"
  ...
  <line x1="15" y1="20.5" x2="17" y2="20.5"/>
    ...
    function renderPanel(notifications) {
        ...
        $header.append('<span>Alertas de Reserva</span>');

        if (notifications.length > 0) {
            const $markAll = $('<button type="button">Marcar tudo como lido</button>');
        ...
        if (notifications.length === 0) {
            $panel.append('<div class="ra-empty">Sem alertas de reserva pendentes.</div>');
    ...
    function renderDisabled() {
        ...
        if (!$panel.length) return;
        $panel.empty();
        $panel.append('<div class="ra-panel-header"><span>Alertas de Reserva</span></div>');
        $panel.append('<div class="ra-empty">Desativado pelo administrador.</div>');
        $badge.removeClass('visible').text('');
    }
    ...
    function firePush(n) {
        ...
        new Notification('Reserva a iniciar em breve', {
            body: `${n.item_name}: ${n.begin}\nReservado por ${n.reserver}`,
