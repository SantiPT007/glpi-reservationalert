...
try {
    ...
    foreach ($rows as $row) {
        ...
        if ($is_test) {
            $item_name = __('[TEST] Test notification', 'reservationalert');
            $begin_str = __('Now', 'reservationalert');
            $reserver  = 'Admin';
        } else {
            $begin_str = '';
            ...
            if (empty($item_name)) {
                $item_name = $itemtype ? $itemtype . ' #' . $items_id : __('Unknown item', 'reservationalert');
            }
