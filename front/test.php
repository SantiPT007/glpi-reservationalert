...
try {
    ...
    if (!$DB->tableExists('glpi_plugin_reservationalert_configs')) {
        throw new \RuntimeException(
            sprintf(__('Table %s does not exist.', 'reservationalert'), 'glpi_plugin_reservationalert_configs') . "\n" .
            __('The plugin needs to be reinstalled: Setup > Plugins > Disable > Enable.', 'reservationalert')
        );
    }

    if (!$DB->tableExists('glpi_plugin_reservationalert_notifications')) {
        throw new \RuntimeException(
            sprintf(__('Table %s does not exist.', 'reservationalert'), 'glpi_plugin_reservationalert_notifications') . "\n" .
            __('The plugin needs to be reinstalled: Setup > Plugins > Disable > Enable.', 'reservationalert')
        );
    }
